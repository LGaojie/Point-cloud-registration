...
template<class Type> class MatrixHx
{
public:
    MatrixHx();
    MatrixHx(unsigned int row, unsigned int col);
    MatrixHx(const MatrixHx<Type>& rhs);
    ~MatrixHx();

    static bool MUL(const MatrixHx<Type>& A, const MatrixHx<Type>& B, MatrixHx<Type>& C);
    static bool DOTMUL(const MatrixHx<Type>& A, const MatrixHx<Type>& B, MatrixHx<Type>& C);
    static MatrixHx<Type> DOTMUL(const MatrixHx<Type>& A, const MatrixHx<Type>& B);
    static bool DOTDIV(const MatrixHx<Type>& A, const MatrixHx<Type>& B, MatrixHx<Type>& C);
    static bool Transpose(const MatrixHx<Type>& A, MatrixHx<Type>& B);
 
    MatrixHx<Type> Transpose() const;
    MatrixHx<Type>& operator = (const MatrixHx<Type>& rhs);
    MatrixHx<Type> operator * (const MatrixHx<Type>& B) const;
    Type*& operator[](unsigned int row);
    const Type* operator[](unsigned int row) const;
    
    MatrixHx<Type> GetRow(unsigned int row) const;
    MatrixHx<Type> GetColumn(unsigned int col) const;
    void Reset(unsigned int row, unsigned int col);

public:
    unsigned int m_nRow;         // 矩阵行数
    unsigned int m_nCol;         // 矩阵列数
    Type**       m_ppData;       // 数据列表（其中的指针指向对应的行的第一个数据）

private:
    Type* m_pData;               // 实际存储的数据
};
